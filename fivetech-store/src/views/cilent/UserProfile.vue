                  ...
                  <div class="order-footer">
                    ...
                    <div class="order-actions">
                      ...
                      <button
                        ...
                        :class="['action-btn', order.items?.every(i => i.is_reviewed) ? 'reviewed' : 'review']"
                        @click="openReviewModal(order)"
                        :disabled="order.items?.every(i => i.is_reviewed)"
                      >
                        {{ order.items?.every(i => i.is_reviewed) ? '✓ Đã đánh giá' : (order.items?.some(i => i.is_reviewed) ? '★ Xem / Đánh giá' : '★ Đánh giá') }}
                      </button>
                    </div>
                  </div>
                </div>
  ...
  <div v-if="reviewModal.open" class="review-overlay" @click.self="closeReviewModal">
    <div class="review-modal">
      ...
      <div class="review-modal-body">
        <div class="review-item" v-for="(item, idx) in reviewModal.items" :key="item.id" :class="{ 'is-reviewed': item.is_reviewed }">
          <div class="review-product">
            ...
            <span v-if="item.is_reviewed" class="reviewed-badge">✓ Đã đánh giá</span>
          </div>
          <div class="star-rating" :class="{ disabled: item.is_reviewed }">
            <span
              v-for="star in 5"
              :key="star"
              class="star"
              :class="{ active: star <= (reviewModal.ratings[idx] || 0) }"
              @click="!item.is_reviewed && (reviewModal.ratings[idx] = star)"
            >★</span>
          </div>
          <textarea
            ...
            rows="3"
          ></textarea>
          <div v-else class="review-content-readonly">
            <p>{{ reviewModal.contents[idx] }}</p>
          </div>
        </div>
      </div>
      <div class="review-modal-footer">
        <button class="btn-cancel" @click="closeReviewModal">Bỏ qua</button>
        <button class="btn-submit-review" @click="submitReviews" :disabled="reviewModal.submitting || reviewModal.items.every(i => i.is_reviewed)">
          {{ reviewModal.items.every(i => i.is_reviewed) ? 'Đã gửi đánh giá' : (reviewModal.submitting ? 'Đang gửi...' : 'Gửi đánh giá') }}
        </button>
      </div>
    </div>
  </div>
...
const openReviewModal = (order) => {
  ...
  reviewModal.value = {
    ...
    items: items,
    ratings: items.map(item => item.review?.rating ?? 5),
    contents: items.map(item => item.review?.content ?? ''),
    submitting: false,
  }
}
...
const submitReviews = async () => {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (items[i].is_reviewed) continue
  ...
  reviewModal.value.open = false
  if (successCount > 0) {
    alert(`Đã gửi ${successCount} đánh giá thành công!`)
    // Refresh orders to update review status
    const ordersRes = await api.get('/orders')
    allOrders.value = ordersRes.data.orders || []
  } else {
    alert('Vui lòng nhập nhận xét cho ít nhất 1 sản phẩm.')
  }
}
...
.review-item.is-reviewed { background: #f8fafc; }
.star-rating.disabled .star { cursor: default; pointer-events: none; }
.reviewed-badge {
  margin-left: auto;
  padding: 4px 10px;
  background: #dcfce7;
...
.review-content-readonly p { margin: 0; }

/* Reviewed button on order card */
.action-btn.reviewed {
  background: #e2e8f0;
  color: #64748b;
  cursor: not-allowed;
